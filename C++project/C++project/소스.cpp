...
//아이템을 떨어트릴때 하나씩 떨어지게 하기위한 구조체랄까여?^^
struct itemstruct
{
	bool itemstart;
	int itemx, itemy;
...
{
	static TCHAR szAppName[] = TEXT("haha");
	HWND hwnd;
	MSG msg;
	WNDCLASS wndclass;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		for (i = 0; i <= 30; i++)//처음엔 전부다 false로 잡아서 안떨어지게...=ㅅ=
		{
	...
	case WM_TIMER:
		for (i = 0; i <= 30; i++)
		{
			if (items[i].itemstart == true) //아이템 떨어지는고
			{
				items[i].itemy += 5;
	...
	case WM_PAINT:
		...
		wsprintf(jumsu, "%3d", jumsucount);
		TextOut(hdc, 0, 0, jumsu, 20);
